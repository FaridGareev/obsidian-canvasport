...
export class settings_tab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(this.containerEl).setName('Transparent background').setDesc('Remove the solid background from PNG, WebP and SVG files.').addToggle((toggle) => toggle.setValue(this.plugin.settings.transparent_background).onChange(async (value) => { this.plugin.settings.transparent_background = value; await this.plugin.save_settings(); }));
