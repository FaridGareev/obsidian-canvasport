...
export function strip_markdown(value: string, keep_urls = false): string {
	...
	return linked.replace(/^#{1,6}\s+/gmu, '').replace(/[`*_~]/gu, '').replace(/^>\s?/gmu, '').replace(/\n{3,}/gu, '\n\n').trim();
}
...
export function render_markdown(value: string): string {
	...
	for (const line of lines) {
		...
		close_list();
		output.push(`<p>${render_inline(line)}</p>`);
	}
	close_list();
	if (code_lines) output.push(`<pre><code>${escape_html(code_lines.join('\n'))}</code></pre>`);
	return output.join('');
...
function render_inline(value: string): string {
	const tokens: string[] = [];
	const protect = (html: string) => { const token = `@@canvas_token_${tokens.length}@@`; tokens.push(html); return token; };
	...
	return rendered.replace(/@@canvas_token_(\d+)@@/gu, (_match, index: string) => tokens[Number(index)] ?? '');
}
