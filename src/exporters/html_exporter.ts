...
import { escape_attribute, escape_html, render_markdown, strip_frontmatter } from '../lib/text';
import type { canvas_edge, canvas_node, canvas_point } from '../models/canvas';
import type { canvas_document } from '../models/canvas';
import type { export_options, export_theme } from '../models/export';

export function render_html_export(document: canvas_document, theme: export_theme, options: export_options): string {
	...
	const group_markup = groups.map((node) => render_group(node, offset_x, offset_y, options.group_title_scale, colors.group_stroke, options.include_group_labels)).join('');
	const node_markup = nodes.map((node) => render_node(node, offset_x, offset_y, theme)).join('');
	const edge_markup = snapshot.canvas.edges.map((edge) => render_edge(edge, snapshot.nodes_by_id, offset_x, offset_y, theme)).join('');
	return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escape_html(options.canvas_name)}</title><style>${render_styles(width, height, theme, colors, options)}</style></head><body><main class="canvas_export">${group_markup}<svg class="canvas_edges" viewBox="0 0 ${width} ${height}" aria-hidden="true"><defs><marker id="canvas_arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M 0 0 L 8 4 L 0 8 z" fill="context-stroke"/></marker></defs>${edge_markup}</svg>${node_markup}</main></body></html>`;
}

function render_group(node: canvas_node, offset_x: number, offset_y: number, title_scale: number, fallback_color: string, include_label: boolean): string {
	const color = resolve_edge_color(node.color);
	...
	return `<section class="canvas_group" style="left:${node.x + offset_x}px;top:${node.y + offset_y}px;width:${node.width}px;height:${node.height}px;border-color:${color || fallback_color}">${label}</section>`;
}

function render_node(node: canvas_node, offset_x: number, offset_y: number, theme: export_theme): string {
	const color = resolve_canvas_color(node.color, theme);
	return `<article class="canvas_node" style="left:${node.x + offset_x}px;top:${node.y + offset_y}px;width:${node.width}px;height:${node.height}px;--node_fill:${color.fill};--node_stroke:${color.stroke};--node_text:${color.text}">${render_node_content(node)}</article>`;
}

function render_node_content(node: canvas_node): string {
	if (node.type === 'text') return render_markdown(node.text ?? '');
	if (node.type === 'file') {
		const file_name = (node.file ?? '').split('/').pop() || 'Untitled file';
		return `<p class="canvas_embed">📄 ${escape_html(file_name)}${node.subpath ? ` <small>${escape_html(node.subpath)}</small>` : ''}</p>`;
	}
	if (node.type === 'link') {
		const url = node.url ?? '';
...
function render_edge(edge: canvas_edge, nodes_by_id: Map<string, canvas_node>, offset_x: number, offset_y: number, theme: export_theme): string {
	...
	const path = `<path d="M ${from.x} ${from.y} C ${controls.first.x} ${controls.first.y}, ${controls.second.x} ${controls.second.y}, ${to.x} ${to.y}" stroke="${color}" marker-end="url(#canvas_arrow)"/>`;
...
function create_document_colors(theme: export_theme): { body: string; grid: string; text: string; group_stroke: string; label_text: string; label_fill: string } {
	return theme === 'dark'
		? { body: '#171b22', grid: 'rgba(255,255,255,.11)', text: '#edf2f7', group_stroke: '#a993d3', label_text: '#18141f', label_fill: '#a993d3' }
		: { body: '#ffffff', grid: 'rgba(31,41,55,.14)', text: '#172033', group_stroke: '#6f7d90', label_text: '#ffffff', label_fill: '#6f7d90' };
}
