...
export function render_svg_export(document: canvas_document, options: export_options): string {
	...
	const groups = snapshot.canvas.nodes.filter((node) => node.type === 'group').map((node) => render_svg_group(node, offset_x, offset_y, options.include_group_labels, options.visual_theme)).join('');
	const edges = snapshot.canvas.edges.map((edge) => {
		...
		return from && to ? render_svg_edge(from, to, edge.fromSide, edge.toSide, edge.color, offset_x, offset_y, options.visual_theme) : '';
	}).join('');
	const nodes = snapshot.canvas.nodes.filter((node) => node.type !== 'group').map((node) => render_svg_node(node, offset_x, offset_y, options.visual_theme)).join('');
	const palette = options.visual_theme === 'dark' ? { background: '#171b22', grid: '#b8c5d8' } : { background: '#ffffff', grid: '#64748b' };
	...
	return `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><pattern id="canvas_grid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="1" fill="${palette.grid}" opacity=".35"/></pattern><marker id="canvas_arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8z" fill="context-stroke"/></marker></defs>${background}${grid}${groups}<g fill="none" stroke-linecap="round" stroke-linejoin="round">${edges}</g>${nodes}</svg>`;
}

function render_svg_group(node: canvas_node, offset_x: number, offset_y: number, include_label: boolean, theme: 'light' | 'dark'): string {
...
function render_svg_node(node: canvas_node, offset_x: number, offset_y: number, theme: 'light' | 'dark'): string {
...
function render_svg_edge(from_node: canvas_node, to_node: canvas_node, from_side: canvas_side | undefined, to_side: canvas_side | undefined, color: string | undefined, offset_x: number, offset_y: number, theme: 'light' | 'dark'): string {
	...
	return `<path d="M ${from.x} ${from.y} C ${controls.first.x} ${controls.first.y}, ${controls.second.x} ${controls.second.y}, ${to.x} ${to.y}" stroke="${resolve_edge_color(color, theme)}" stroke-width="2" marker-end="url(#canvas_arrow)"/>`;
}
